 ...
 * @param[in] enable_flush_done call the LLUI_DISPLAY_flushDone() function when the copy is done.
 */
void DRAWING_DMA2D_configure_memcpy(uint8_t* srcAddr, uint8_t* destAddr, uint32_t xmin, uint32_t ymin, uint32_t xmax, uint32_t ymax, uint32_t stride, DRAWING_DMA2D_memcpy* memcpy_data, void * callback_function);
/*
